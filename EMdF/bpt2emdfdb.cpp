 ...
 * Sandborg-Petersen at ulrikp-at-emergence*dot*dk for more
 * information about the BPT2 engine.
 *
 * Ulrik Petersen
 * Created: 1/27-2001
...
/**@file bpt2emdfdb.cpp
 *@brief Implementation of BPT2EMdFDB (EMdF layer)
 */
...
/////////////////////////////////////////////////////////
//
// BPT2EMdFDB class implementation
//
/////////////////////////////////////////////////////////
 ...
 *
 */
BPT2EMdFDB::BPT2EMdFDB() 
	: EMdFDB(kBPT2)
{
}

BPT2EMdFDB::BPT2EMdFDB(std::string database_name, std::string key) 
	: EMdFDB(kBPT2)
{
 ...
 */
void BPT2EMdFDB::databaseNameIsOK(const std::string& db_name, bool& bIsOK)
{
	UNUSED(db_name);
	bIsOK = false;
}
 ...
 */
bool BPT2EMdFDB::createDatabase(const std::string& db_name, const std::string& key)
{
	UNUSED(db_name);
	UNUSED(key);
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::createDatabase(const std::string& db_name, eCharsets charset, const std::string& key)
{
	UNUSED(db_name);
	UNUSED(charset);
	UNUSED(key);
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::initializeDatabase(const std::string& db_name, const std::string& key)
{
	UNUSED(db_name);
	UNUSED(key);
 ...
 * @param key is ignored in BPT2EMdFDB.  May be used by derived subclass.
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::useDatabase(const std::string& db_name, const std::string& key)
{
	UNUSED(db_name);
	UNUSED(key);
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::useDatabase(const std::string& db_name, const std::string& key, bool bIsInitializingOrCreating)
{
	UNUSED(db_name);
	UNUSED(key);
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::dropDatabase(const std::string& db_name)
{	
	UNUSED(db_name);
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::dbIsInitialized(bool& bIsInitialized)
{
	UNUSED(bIsInitialized);
	
	DEBUG_X_IS_WRONG("BPT2EMdFDB::useDatabase", "The BPT2 backend is not present in the Open Source version of Emdros.\nFor licensing the BPT2 backend, please contact Ulrik Sandborg-Petersen:\nulrikp-at-emdros{dot}org ... thanks!");
	return false;
}
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::createObjectTypeOT_objects(const std::string& object_type_name,
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::createObjectType(const std::string& object_type_name,
				  const std::list<FeatureInfo>& FeatureInfos,
				  eObjectRangeType objectRangeType,
				  eMonadUniquenessType monadUniquenessType,
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::dropObjectType(const std::string& object_type_name, id_d_t object_type_id)
 ...
 */
bool BPT2EMdFDB::getLargestObjectLengthFromOT_objects(const std::string& object_type_name,
						      eObjectRangeType objectRangeType,
 ...
 * @return True on no DB error, false otherwise.
 */
bool BPT2EMdFDB::getLargestObjectLength(const std::string& object_type_name,
					eObjectRangeType objectRangeType,
					/* out */ monad_m& largest_object_length)
{
	UNUSED(object_type_name);
 ...
 * @return True on no DB error, false otherwise.
 */
bool BPT2EMdFDB::setLargestObjectLengthIfNecessary(const std::string& object_type_name,
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::objectTypeExists(id_d_t object_type_id,
				 /* out */ bool& result, 
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::objectTypeExists(const std::string& object_type_name, 
				 /* out */ bool& result, 
				 /* out */ id_d_t& object_type_id,
				 /* out */ eObjectRangeType& objectRangeType,
				 /* out */ eMonadUniquenessType& monadUniquenessType) 
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::createEnum(const std::string& enum_name, 
			   /* out */ id_d_t& enum_id)
{
	UNUSED(enum_name);
	UNUSED(enum_id);
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::dropEnum(id_d_t enum_id)
{
	UNUSED(enum_id);
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::enumExists(id_d_t enum_id,
			   /* out */ bool& result,
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::enumExists(const std::string& enum_name, 
			   /* out */ bool& result,
			   /* out */ id_d_t& enum_id)
{
	UNUSED(enum_name);
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::getDefault(id_d_t enum_id, 
			   /* out */ bool& result,
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::createEnumConstant(id_d_t enum_id,
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::setDefaultEnumConst(id_d_t enum_id,
				     const std::string& enum_const_name)
{
	UNUSED(enum_id);
	UNUSED(enum_const_name);
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::createFeature(const std::string& feature_name,
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::addFeatureToOT_objects(const std::string& object_type_name,
					id_d_t object_type_id,
					eObjectRangeType objectRangeType,
					eMonadUniquenessType monadUniquenessType,
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::addFeature(const std::string& object_type_name,
			    id_d_t object_type_id,
			    const FeatureInfo& fi)
{
	UNUSED(object_type_name);
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::dropFeatureFromOT_objects(const std::string& object_type_name,
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::dropFeature(const std::string& object_type_name,
			     const std::string& feature_name,
			     id_d_t feature_type_id,
			     id_d_t object_type_id)
{	
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::featureExists(const std::string& feature_name,
			      id_d_t object_type_id,
			      /* out */ bool& result,
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::getFeatures(const std::string& object_type_name,
			    id_d_t object_type_id,
			     const std::list<FeatureInfo>& FeatureInfos,
			     id_d_t object_id_d,
 ...
 *
 * Uses BPT2EMdFDB::getFeaturesByQuery().
 * 
 * @see BPT2EMdFDB::getFeaturesByQuery(), GetFeaturesStatement::exec().
 *
 * @param object_type_name The name of the object type of the object to query.
 *
 * @param object_type_id The id_d of the object type.
 ...
 */
bool BPT2EMdFDB::loadStringSetIntoCacheBPT2(const std::string& object_type_name, 
					    id_d_t object_type_id,
					    const std::string& feature_name,
 ...
 * encoded with BPT2EMdFDB::escapeStringForSQL().
 *
 * @param original_string The string to be created. Must be the original
 * string, before encoding with BPT2EMdFDB::escapeStringForSQL().
 *
 * @param result The set id_d value created
 *
 * @return true on found, false on not found or DB error.
 */
bool BPT2EMdFDB::addStringToStringSetTable(const std::string& normalized_object_type_name,
 ...
 * @return true on found, false on not found or DB error.
 */
bool BPT2EMdFDB::getID_DFromStringSet(const std::string& normalized_object_type_name,
				     id_d_t object_type_id,
				     const std::string& encoded_feature_name,
 ...
 * @return true on no DB error, false on DB error.
 */
bool BPT2EMdFDB::createStringSetTable(const std::string& object_type_name, 
				      const std::string& feature_name)
{
	UNUSED(object_type_name);
	UNUSED(feature_name);
 ...
 * @return true on no DB error, false on DB error.
 */
bool BPT2EMdFDB::createIndicesOnStringSetTable(const std::string& object_type_name,
					       const std::string& feature_name)
{
	UNUSED(object_type_name);
	UNUSED(feature_name);
 ...
 * @return true on no DB error, false on DB error.
 */
bool BPT2EMdFDB::dropIndicesOnStringSetTable(const std::string& object_type_name,
					     const std::string& feature_name)
{
	UNUSED(object_type_name);
	UNUSED(feature_name);
 ...
 * @return true on no DB error, false on DB error.
 */
bool BPT2EMdFDB::dropStringSetTable(const std::string& object_type_name, 
				    const std::string& feature_name)
{
	UNUSED(object_type_name);
	UNUSED(feature_name);
 ...
 *@internal
 *
 * This just calls BPT2EMdFDB::createMonadSet with the segment name and a
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::createSegment(const std::string& segment_name, 
...
bool BPT2EMdFDB::insertMonadRange(monad_m first, monad_m last)
{
	UNUSED(first);
	UNUSED(last);
...
bool BPT2EMdFDB::insertMonadRangeOT(const std::string& object_type_name, monad_m first, monad_m last)
{
	UNUSED(object_type_name);
	UNUSED(first);
	UNUSED(last);
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::selectMonadSets(/* out */ std::list<std::string>& monad_set_names)
{
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::monadSetExists(const std::string& monad_set_name, 
				/* out */ bool& bExists,
				/* out */ SetOfMonads& som)
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::createMonadSet(const std::string& monad_set_name, const SetOfMonads& som)
{
	UNUSED(monad_set_name);
	UNUSED(som);
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::dropMonadSet(const std::string& monad_set_name)
{
	UNUSED(monad_set_name);
 ...
 */
bool BPT2EMdFDB::connectionOk(void)
{
	DEBUG_X_IS_WRONG("BPT2EMdFDB::", "The BPT2 backend is not present in the Open Source version of Emdros.\nFor licensing the BPT2 backend, please contact Ulrik Sandborg-Petersen:\nulrikp-at-emdros{dot}org ... thanks!");
	return false;
}
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::getObjectsHavingMonadsInMonadSet(const std::string object_type_name,
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::getObjectsHavingMonadsIn(const std::string object_type_name,
					  id_d_t object_type_id,
					  eObjectRangeType objectRangeType,	
					  const SetOfMonads& monad_ms,
 ...
 *
 * This is the entry point into BPT2EMdFDB; the other getInst() is used by
 * this method.
 *
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::getInst(const std::string& object_type_name,
			id_d_t object_type_id,
 ...
 *
 * This is not the public entry point into BPT2EMdFDB; the other getInst()
 * uses this method.
 *
 * @param object_type_name the name of the object type
 * 
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::getInstLocalBPT2(const std::string& object_type_name,
				  id_d_t object_type_id,
				  const SetOfMonads& Su,
				  const SetOfMonads& all_m_1,
				  EMdFFFeatures *pre_query_constraints,
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::getObjectsStartingAtSm(const std::string& object_type_name,
					monad_m Sm,
					/* out */ IDDList& Result)
{
	UNUSED(object_type_name);
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::getMonadsFromID_D(id_d_t object_id_d, 
				  id_d_t object_type_id,
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::getMonadsFromID_Ds(const SetOfMonads& id_ds_set,
				    const std::string &object_type_name,
				    eObjectRangeType objectRangeType,
				    Table *pOutTable)
{
 ...
 *@internal
 *
 * This is similar to BPT2EMdFDB::getMonadsFromID_D(), but that method has
 * fewer parameters, and actulaly calls this method.
 *
 * The rationale for having both is that BPT2EMdFDB::getMonadsFromID_D()
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::getSOMForObject(const std::string& object_type_name,
				 id_d_t object_id_d,
				 eObjectRangeType objectRangeType,
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::getObjectTypes(/* out */ std::list<std::string>& Result)
{
	UNUSED(Result);
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::getFeaturesForObjectType(id_d_t object_type_id,
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::getEnumerations(/* out */ std::list<std::string>& Result)
{
	UNUSED(Result);
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::loadEnumConstantsIntoCache(id_d_t enum_id)
{
	UNUSED(enum_id);
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::getEnumConstants(id_d_t enum_id,
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::getObjectTypesUsingEnumeration(id_d_t enum_id,
 ...
 *@internal
 *
 * This is different from BPT2EMdFDB::getObjectsHavingMonadsInMonadSet()
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::getObjectsWithinMonads(const SetOfMonads& monads, 
					const std::string& object_type_name,
					eObjectRangeType objectRangeType,
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::objectID_DExistsInType(id_d_t object_id_d, 
					const std::string& object_type_name,
					/* out */ bool& Result)
{
	UNUSED(object_id_d);
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::objectID_DExists(id_d_t object_id_d, 
				 /* out */ bool& Result,
				 /* out */ id_d_t& object_type_id)
{
	UNUSED(object_id_d);
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::createObject(id_d_t object_id_d,
			      const std::string& object_type_name,
			      id_d_t object_type_id,
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::updateObject(id_d_t object_id_d,
			      const std::string& object_type_name,
			      id_d_t object_type_id,
			      const std::list<FeatureInfo>& features)
{
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::dropObject(id_d_t object_id_d, 
			    id_d_t object_type_id, 
			    const std::string& object_type_name,
			    eObjectRangeType objectRangeType)
{
 ...
 *@internal
 *
 * Uses BPT2EMdFDB::getObjectsWithinMonads() to get the object id_ds to
 * drop.
 *
 ...
 * @return true on no DB error, false otherwise.
 */
bool BPT2EMdFDB::dropObjectsInMonads(const SetOfMonads& monads, 
				     const std::string& object_type_name, 
				     id_d_t object_type_id, 
				     eObjectRangeType objectRangeType,
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::createObjects(const std::string& object_type_name,
			       id_d_t object_type_id,
 ...
 * OBJECT TYPE.".  It is used by BPT2EMdFDB::createObjects() to create one
 * object's worth of output data in a std::string, to be sent to the
 * backend later by BPT2EMdFDB::createObjectsOT_objects_DB() (or its
 ...
 * @return True on no database error, false if a database error occurred.
 */
void BPT2EMdFDB::createObjectsOT_objects_data(const std::string object_type_name, 
					      id_d_t object_type_id,
 ...
 *
 * This is part of the machinery for the MQL "CREATE OBJECTS WITH
 * OBJECT TYPE.".  It is used by BPT2EMdFDB::createObjects() to send the
 * current batch of objects to the backend.
 *
 * @see BPT2EMdFDB::createObjects()
 *
 * @param object_type_name The name of the object type to contain the
 * objects.
 *
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::createObjectsOT_objects_DB(const std::string& object_type_name,
 ...
 *
 * This is part of the machinery for the MQL "CREATE OBJECTS WITH
 * OBJECT TYPE.".  It is used by BPT2EMdFDB::createObjects() and others to
 ...
 */
void BPT2EMdFDB::createObjectsInBetweenData(/* out */ std::string& OT_data)
{
	UNUSED(OT_data);	
 ...
 * BPT2EMdFDB::ugpradeDatabase() method is able to upgrade the database
 ...
 * @return True on no DB error, false otherwise.
 */
bool BPT2EMdFDB::getCurrentDatabaseVersion(const std::string& database_name, 
 ...
 * the database.  Use BPT2EMdFDB::getCurrentDatabaseVersion() to get the
 * number.
 *
 * @return True on no DB error, false otherwise.
 */
bool BPT2EMdFDB::upgradeDatabase(const std::string& database_name, 
				 int nCurrentVersion)
{
	UNUSED(database_name);
	UNUSED(nCurrentVersion);
 ...
 * true.
 */
bool BPT2EMdFDB::objectTypeHasObjects(const std::string& object_type_name,
 ...
 * true.
 */
bool BPT2EMdFDB::tableExists(std::string table_name, bool&  /* out */ bTableExists)
{
	UNUSED(table_name);
	UNUSED(bTableExists);
 ...
 * @return True on no DB error, false otherwise.
 */
bool BPT2EMdFDB::createMin_max_m_table(void)
{
	DEBUG_X_IS_WRONG("BPT2EMdFDB::", "The BPT2 backend is not present in the Open Source version of Emdros.\nFor licensing the BPT2 backend, please contact Ulrik Sandborg-Petersen:\nulrikp-at-emdros{dot}org ... thanks!");
	return false;
}
 ...
 * BPT2EMdFDB::setMin_max_m_fromObjectType() for each object type.
 *
 ...
 * @return True on no DB error, false otherwise.
 */
bool BPT2EMdFDB::setMin_max_m_fromObjectTypes(bool bBeVerbose)
{
	UNUSED(bBeVerbose);
 ...
 * @return True on no DB error, false otherwise.
 */
bool BPT2EMdFDB::setMin_max_m_fromObjectType(const std::string& object_type_name, 
					     bool bBeVerbose)
{
	UNUSED(object_type_name);
	UNUSED(bBeVerbose);
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::getMax_m(monad_m& /* out */ max_m)
{
	UNUSED(max_m);
 ...
 * @return True on no DB error, false otherwise.
 */
bool BPT2EMdFDB::setMax_m(monad_m max_m_candidate, bool bSetUnconditionally)
{
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::getMin_m(monad_m& /* out */ min_m)
{
	UNUSED(min_m);
 ...
 * @return True on no DB error, false otherwise.
 */
bool BPT2EMdFDB::setMin_m(monad_m min_m_candidate, bool bSetUnconditionally) 
{
 ...
 * @return True on no DB error, false otherwise.
 */
bool BPT2EMdFDB::createIndex(const std::string& index_name, 
 ...
 * @return True on no DB error, false otherwise.
 */
bool BPT2EMdFDB::createIndicesOnOTObjects(const std::string& object_type_name) 
{
	UNUSED(object_type_name);
 ...
 *
 * First uses the database by calling BPT2EMdFDB::useDatabase() (or its
 * polymorphic instance), then getting a list of object types in the
 * database, then calling BPT2EMdFDB::createIndicesOnObjectType() for each
 * of them.
 *
 * @see BPT2EMdFDB::useDatabase(), BPT2EMdFDB::getObjectTypes(),
 * BPT2EMdFDB::createIndicesOnObjectType().
 *
 * Note that the encryption key is only used for SQLite, and only if
 * you have an SQLite that supports encryption.
 *
 * @param database_name The database to use and create indexes on.
 * @param key Just passed to BPT2EMdFDB::useDatabase()
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::createIndicesOnDatabase(const std::string& database_name, const std::string& key)
{
	UNUSED(database_name);
	UNUSED(key);
 ...
 * First uses the database by calling BPT2EMdFDB::useDatabase() (or its
 * polymorphic instance), then getting a list of object types in the
 * database, then calling BPT2EMdFDB::dropIndicesOnObjectType() for each
 * of them.
 *
 * Note that the encryption key is only used for SQLite, and only if
 * you have an SQLite that supports encryption.
 *
 * @see BPT2EMdFDB::useDatabase(), BPT2EMdFDB::getObjectTypes(),
 * BPT2EMdFDB::dropIndicesOnObjectType().
 *
 * @param database_name The database to use and drop indexes on.
 * @param key Just passed to BPT2EMdFDB::useDatabase()
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::dropIndicesOnDatabase(const std::string& database_name, const std::string& key)
{
	UNUSED(database_name);
	UNUSED(key);
 ...
 * BPT2EMdFDB::createIndicesOnObjectType(const std::string&).
 *
 * Note that the encryption key is only used for SQLite, and only if
 * you have an SQLite that supports encryption.
 *
 * @see BPT2EMdFDB::useDatabase(), BPT2EMdFDB::createIndicesOnObjectType(const
 * std::string&).
 *
 * @param database_name The database to use and drop indexes on.
 * @param key Just passed to BPT2EMdFDB::useDatabase()
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::createIndicesOnObjectType(const std::string& database_name, const std::string& key, const std::string& object_type_name)
{
	UNUSED(database_name);
	UNUSED(key);
	UNUSED(object_type_name);
 ...
 * First uses the database by calling BPT2EMdFDB::useDatabase() (or its
 * polymorphic instance), then calling
 * BPT2EMdFDB::dropIndicesOnObjectType(const std::string&).
 *
 * Note that the encryption key is only used for SQLite, and only if
 * you have an SQLite that supports encryption.
 *
 * @see BPT2EMdFDB::useDatabase(), BPT2EMdFDB::dropIndicesOnObjectType(const
 * std::string&).
 *
 * @param database_name The database to use and drop indexes on.
 * @param key Just passed to BPT2EMdFDB::useDatabase()
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::dropIndicesOnObjectType(const std::string& database_name, const std::string& key, const std::string& object_type_name)
{
	UNUSED(database_name);
	UNUSED(key);
	UNUSED(object_type_name);
 ...
 *
 * Calls BPT2EMdFDB::createIndicesOnOTObjects() (or its polymorphic variant).
 *
 * @see BPT2EMdFDB::createIndicesOnOTObjects(),
 *
 * @param object_type_name Name of the object type on which to create default indexes.
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::createIndicesOnObjectType(const std::string& object_type_name)
{
	UNUSED(object_type_name);
 ...
 *
 * Calls BPT2EMdFDB::dropIndicesOnOTObjects(), and, if necessary,
 * BPT2EMdFDB::dropIndicesOnOTMonadMs() (or their polymorphic variants).
 *
 * @see BPT2EMdFDB::dropIndicesOnOTObjects(),
 * BPT2EMdFDB::dropIndicesOnOTMonadMs().
 *
 * @param object_type_name Name of the object type on which to drop default indexes.
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::dropIndicesOnObjectType(const std::string& object_type_name)
{
	UNUSED(object_type_name);
 ...
 * @return True on no database error, false if a database error occurred.
 */
bool BPT2EMdFDB::dropIndicesOnOTObjects(const std::string& object_type_name)
{
	UNUSED(object_type_name);
 ...
 * @return True on no database error, false if a database error occurred.
 */
void BPT2EMdFDB::dropIndex(const std::string& index_name, const std::string& table_name)
{
	UNUSED(index_name);
	UNUSED(table_name);
 ...
 */
EMdFComparison* BPT2EMdFDB::getEMdFComparison(const std::string& left_feature_name, 
...
EMdFFFactor* BPT2EMdFDB::getEMdFFFactor(EMdFFFactor* ffactor)
{
	UNUSED(ffactor);
...
EMdFFFactor* BPT2EMdFDB::getEMdFFFactor(EMdFFFeatures* ffeatures)
{
	UNUSED(ffeatures);
...
EMdFFFactor* BPT2EMdFDB::getEMdFFFactor(EMdFComparison* feature_comparison)
{
...
EMdFFTerm* BPT2EMdFDB::getEMdFFTerm(EMdFFTerm* fterm, EMdFFFactor* ffactor)
{
	UNUSED(fterm);
	UNUSED(ffactor);
...
EMdFFFeatures* BPT2EMdFDB::getEMdFFFeatures(EMdFFFeatures* ffeatures, EMdFFTerm* fterm)
{
	UNUSED(ffeatures);
	UNUSED(fterm);
 ...
 */
bool BPT2EMdFDB::canDoComparisonOp(eComparisonOp e, eEVkind kind)
{
	UNUSED(e);
	UNUSED(kind);
 ...
 */
std::string BPT2EMdFDB::escapeStringForSQL(const std::string& str)
{
	UNUSED(str);
...
bool BPT2EMdFDB::canDeclareEncoding(eCharsets charset) const
{
	UNUSED(charset);
...
BPT2EMdFConnection *BPT2EMdFDB::getConnection(void)
{
	DEBUG_X_IS_WRONG("BPT2EMdFDB::", "The BPT2 backend is not present in the Open Source version of Emdros.\nFor licensing the BPT2 backend, please contact Ulrik Sandborg-Petersen:\nulrikp-at-emdros{dot}org ... thanks!");
	return 0;
}

std::string BPT2EMdFDB::FeatureInfo2SQLvalue(const std::string& OTN,
					     id_d_t object_type_id,
...
bool BPT2EMdFDB::getStringSetStrings(const std::string& object_type_name, 
				     id_d_t object_type_id,
				     const std::string& feature_name,
				     /* out */ Table& result)
{
