...
bool PennTreebankImporter::readDocumentFromStream(std::istream *pIn)
{
	...
	if (bResult && m_pCorpus != 0 && !m_pCorpus->isEmpty()) {
		...
		++m_cur_id_d;
		m_cur_docid = 0;
		emdros_int64 doc_docid = m_cur_docid;
		++m_cur_docid;
...
void doAddCorefToMap(PennTreeNode *pNode, CorefMap& mymap, bool bUseIntegerDocIDs)
{
	if (pNode->getStrCoref() != "") {
		...
		if (bUseIntegerDocIDs) {
			emdros_int64 mydocid = pNode->getDocID();
			mymap[coref].push_back(mydocid);
		} else {
			emdros_int64 mydocid = pNode->getID_D();
			mymap[coref].push_back(mydocid);
		}
	} 
}
...
void doAddCorefsToNode(PennTreeNode *pNode, CorefMap& mymap, bool bUseIntegerDocIDs)
{
	if (pNode->getStrCoref() != "") {
		...
		const std::list<emdros_int64>& mylist = mymap[coref];
		std::list<emdros_int64>::const_iterator ci = mylist.begin();
		std::list<emdros_int64>::const_iterator cend = mylist.end();
		while (ci != cend) {
			if (*ci != mydocid) {
...
EmdrosMemObject *PennTreebankImporter::createDoc(monad_m first, monad_m last, id_d_t id_d, emdros_int64 doc_docid)
{
...
SetOfMonads PennTreebankImporter::assignMonadsAndID_Ds(PennTreeNode *pNode, SetOfMonads& sibling_monads, id_d_t parent_id_d, emdros_int64 parent_docid)
{
	if (pNode->getType() == kPennTerminal) {
