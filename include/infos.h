...
private:
	...
	bool m_is_default;
public:
	EnumConstInfo(std::string name, emdros_int64 value, bool is_default);
	EnumConstInfo(const EnumConstInfo& other);
	~EnumConstInfo();
	const std::string& getName(void) const;
	emdros_int64 getValue(void) const;
	bool getIsDefault(void) const;
	bool operator==(const EnumConstInfo& other) const;
	void setValue(emdros_int64 new_value) { m_value = new_value; };
