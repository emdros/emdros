 ...
 * negraimport.cpp
 *
 * A front-end to the Negra import
...
int exec_list(const std::list<std::string>& input_filename_list, std::ostream* pOutStream, const std::string& dbname, monad_m start_monad, id_d_t start_id_d, bool bPrintSchema)
{
	...
	NEGRAImporter *pImporter = new NEGRAImporter(start_monad, start_id_d);
...
{
	ostr << "negraimport from Emdros version " << EMDROS_VERSION << std::endl;
}
...
void print_usage(std::ostream& ostr)
{
	show_version(ostr);
	ostr << "Usage:" << std::endl;
	ostr << "   negraimport [options] [inputfile ...]" << std::endl;
	ostr << "   negraimport (--help|-V|--version)" << std::endl;
