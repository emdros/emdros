 ...
 * plaintextimport.cpp
 *
 * A front-end to Emdros for importing plain text
...
int exec_list(const std::list<std::string>& input_filename_list, std::ostream* pOutStream, const std::string& dbname, monad_m start_monad, id_d_t start_id_d, bool bPrintSchema)
{
	int result = 0;

	PlainTextImporter *pImporter = new PlainTextImporter(start_monad, start_id_d);
...
{
	ostr << "plaintextimport from Emdros version " << EMDROS_VERSION << std::endl;
}
...
void print_usage(std::ostream& ostr)
{
	show_version(ostr);
	ostr << "Usage:" << std::endl;
	ostr << "   plaintextimport [options] [inputfile ...]" << std::endl;
	ostr << "   plaintextimport (--help|-V|--version)" << std::endl;
...
int main(int argc, char* argv[])
{
	...
	try {
		...
		if (bContinue) {
			nResult = exec_list(input_filename_list, pOut, db, start_monad, start_id_d, bPrintSchema);
		}
