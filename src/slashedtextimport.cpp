...
int exec_list(const std::list<std::string>& input_filename_list, std::ostream* pOutStream, const std::string& dbname, monad_m start_monad, id_d_t start_id_d, const std::string& word_internal_separator, const std::string& paragraph_separator_string, bool bPrintSchema)
{
	int result = 0;

	SlashedTextImporter *pImporter = new SlashedTextImporter(start_monad, start_id_d, word_internal_separator, paragraph_separator_string);
...
{
	ostr << "slashedtextimport from Emdros version " << EMDROS_VERSION << std::endl;
}
...
void print_usage(std::ostream& ostr)
{
	show_version(ostr);
	ostr << "Usage:" << std::endl;
	ostr << "   slashedtextimport [options] [inputfile ...]" << std::endl;
	ostr << "   slashedtextimport (--help|-V|--version)" << std::endl;
	...
	ostr << "   --word-sep string    The string (default '/') separating words internally" << std::endl;
	ostr << "   --para-sep string    The string separating paragraphs." << std::endl;
	ostr << "DEFAULTS:" << std::endl;
	ostr << "   -o -                 (Use stdout)" << std::endl;
	ostr << "   --word-sep '/'" << std::endl;
	ostr << "   --para-sep '\\n\\n'" << std::endl;
...
int main(int argc, char* argv[])
{
	...
	std::string word_internal_separator;
	std::string paragraph_separator_string;
	...
	addOption("--word-sep", "--word-sep", true, 
		  "/", // Default value
		  "ERROR: --word-sep must have a string as its argument\n"
		  "       example: --word-sep '/'\n");

	addOption("--para-sep", "--para-sep", true, 
		  "\n\n", // Default value
		  "ERROR: --para-sep must have a string as its argument\n"
		  "       example: --para-sep '\\n\\n'\n");
	...
	if (!parseArguments(argc, argv, error_message, surplus_arguments)) {
		...
		getArgumentValue("--word-sep", word_internal_separator);
		word_internal_separator = parse_C_backslashes(word_internal_separator);

		// Paragraph-separator string
		getArgumentValue("--para-sep", paragraph_separator_string);
		paragraph_separator_string = parse_C_backslashes(paragraph_separator_string);
	...
	try {
		...
		if (bContinue) {
			nResult = exec_list(input_filename_list, pOut, db, start_monad, start_id_d, word_internal_separator, paragraph_separator_string, bPrintSchema);
		}
