 ...
 * tigerxmlimport.cpp
 *
 * A front-end to the TIGER XMOL import
...
int exec_list(const std::list<std::string>& input_filename_list, std::ostream* pOutStream, const std::string& dbname, monad_m start_monad, id_d_t start_id_d, bool bPrintSchema, const std::string& nonterminal_object_type_feature, const std::string& terminal_object_type_name)
{
	int result = 0;

	TIGERXMLImporter *pImporter = new TIGERXMLImporter(start_monad, start_id_d,
							   nonterminal_object_type_feature,
							   terminal_object_type_name);
...
{
	ostr << "tigerxmlimport from Emdros version " << EMDROS_VERSION << std::endl;
}
...
void print_usage(std::ostream& ostr)
{
	show_version(ostr);
	ostr << "Usage:" << std::endl;
	ostr << "   tigerxmlimport [options] [inputfile]" << std::endl;
	ostr << "   tigerxmlimport (--help|-V|--version)" << std::endl;
	ostr << "OPTIONS:" << std::endl;
	ostr << "   --help               Show this help" << std::endl;
	ostr << "   -V , --version       Show version" << std::endl;
	ostr << "   -s , --schema        Print schema" << std::endl;
...
int main(int argc, char* argv[])
{
	...
	std::string terminal_object_type_name;
	std::string nonterminal_object_type_feature;
	...
	addOption("--ntotf", "--ntotf", true,
		  "", // Default value
		  "ERROR: --ntotf must have a feature name as its argument.\n"
		  "       example: --ntotf cat\n");

	addOption("--termotn", "--termotn", true,
		  "", // Default value
		  "ERROR: --termotn must have an object type as its argument.\n"
		  "       example: --termotn Token\n");
	...
	if (!parseArguments(argc, argv, error_message, surplus_arguments)) {
		...
		// --ntotf nonterminal-object-type-feature
		getArgumentValue("--ntotf", nonterminal_object_type_feature);

		// --termotn terminal-object-type-name
		getArgumentValue("--termotn", terminal_object_type_name);
		...
		if (input_filename_list.size() >= 2) {
			print_usage(std::cerr);
			std::cerr << "ERROR: You can have at most one filename on the command line." << std::endl;
			return 1;
		}
	}
	...
	try {
		...
		if (bContinue) {
			nResult = exec_list(input_filename_list, pOut, db, start_monad, start_id_d, bPrintSchema, nonterminal_object_type_feature, terminal_object_type_name);
		}
